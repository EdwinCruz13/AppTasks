...
export const UserAuth = (req, resp, next) => {
  try {
    ...
    if (!token) {
        ...
        return resp.status(401).json({ error: "Unathorized request" });
    }

    //use webtoken in order to verify the cookie
    jwt.verify(token, process.env.WEBTOKEN_SECRET, (err, user) => {
      if (err) {
        return resp.status(403).json({ error: "Invalid user" });
      }
...
export const AdminAuth = async (req, resp, next) => {
  ...
  if (!user.isAdmin)
    return resp.status(403).json({ error: "Your do not have privilege." });
